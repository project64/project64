...
* Logging Functions                                                       *
**************************************************************************/
void WriteX86Comment ( LPCSTR Comment ) {
	CPU_Message("");
	CPU_Message("      // %s",Comment);
}

void WriteX86Label ( LPCSTR Label ) {
	CPU_Message("");
	CPU_Message("      %s:",Label);
}
...
void MoveX86PointerToX86regDisp(int x86reg, int X86Pointer, BYTE Disp) {
	...
	CPU_Message("      mov %s, dword ptr [%s] + %d",x86_Name(x86reg),x86_Name(X86Pointer),Disp);
